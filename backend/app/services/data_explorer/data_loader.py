...
class DataLoader:
    ...
    def _get_preview(self, rows: int = 5) -> Optional[Dict[str, Any]]:
        """获取数据预览
        ...
        return {
            "head": self._current_data.head(rows).to_dict('records'),
            "info": self._data_info,
            "summary": self._current_data.describe().to_dict()
        } 
    ...
    def _update_data_info(self) -> None:
        ...
        if self._current_data is not None:
            self._data_info = {
                "行数": len(self._current_data),
                "列数": len(self._current_data.columns),
                "列名": list(self._current_data.columns),
                "数据类型": {str(k): str(v) for k, v in self._current_data.dtypes.items()},
                "内存占用": f"{self._current_data.memory_usage().sum() / 1024**2:.2f} MB",
                "空值统计": self._current_data.isnull().sum().to_dict()
            }
    
    
    def _load_dataframe(self, df: pd.DataFrame) -> Dict[str, Any]:
    ...
    def load_csv(self, file_path: str, encoding: str = 'utf-8', sheet_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            result = self._load_dataframe(df)
